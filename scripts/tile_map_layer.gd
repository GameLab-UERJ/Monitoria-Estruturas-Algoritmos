...
func centralizar_camera():
	...
	var centro_x = global_position.x + (grid_largura / 2.0)
	var centro_y = global_position.y + (grid_altura / 2.0)
	

	var deslocamento = 250 
	
	camera.global_position = Vector2(centro_x + deslocamento, centro_y)
	camera.make_current()
